  ...
  return (
    <div className="space-y-6">
      {currentReviews.map((review) => (
        <div key={review.id} className="bg-slate-900/40 p-6 rounded-xl border border-slate-800 group hover:border-slate-700 transition-all">
            ...
            {editingId === review.id ? (
              <div className="space-y-3">
                ...
                <div className="flex gap-2 justify-end">
                  <button onClick={() => setEditingId(null)} className="text-xs text-slate-400 hover:text-white px-3 py-1">Cancel</button>
                  <button onClick={() => handleSaveEdit(review.id)} className="text-xs bg-pink-600 text-white px-3 py-1 rounded-md hover:bg-pink-500">Save Changes</button>
                </div>
              </div>
            ) : (
              <>
